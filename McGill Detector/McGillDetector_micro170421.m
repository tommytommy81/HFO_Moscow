...
function [sig ] = McGillDetector_micro161226(data, p)
...
[HFOobj, results] = nbt_doHFOmicro161226(sig,sig.signalFilt, p.hp, 'Ripple', p, input);  %%%%%%%%%%%%%%%%%%%%%%%DIFFERENT
      % limits for R and FR are 10 times larger (300 and 200 uVpp)
sig.THR = HFOobj.THR;
...
if exist('results', 'var')==1
    for iDet=1:length(results)
        if iDet==1
            ...
            sig.autoSta_s = results(iDet).start;
            sig.autoEnd_s = results(iDet).stop;
            sig.mark = 1;
            
        else
            sig.autoSta     = [sig.autoSta results(iDet).start/p.fs];
            sig.autoEnd     = [sig.autoEnd results(iDet).stop/p.fs];
            sig.autoSta_s  = [sig.autoSta_s results(iDet).start];
            sig.autoEnd_s = [sig.autoEnd_s results(iDet).stop];
            sig.mark = [sig.mark 1];
        end
    end
else
...
[HFOobj, results] = nbt_doHFOmicro161226(sig,sig.signalFiltFR, p.hpFR, 'FastRipple', p, input);
...
if exist('results', 'var')==1
    for iDet=1:length(results)
        if iDet==1
            ...
            temp.autoSta_s = results(iDet).start;
            temp.autoEnd_s = results(iDet).stop;
            temp.mark = 2;
            
        else
            temp.autoSta     = [temp.autoSta results(iDet).start/p.fs];
            temp.autoEnd     = [temp.autoEnd results(iDet).stop/p.fs];
            temp.autoSta_s  = [temp.autoSta_s results(iDet).start];
            temp.autoEnd_s = [temp.autoEnd_s results(iDet).stop];
...
[sig.autoSta_s, sorted_inds] = sort([sig.autoSta_s temp.autoSta_s(Ind==0)]);
sig.autoEnd_s = [sig.autoEnd_s temp.autoEnd_s(Ind==0)];
sig.autoEnd_s  = sig.autoEnd_s(sorted_inds) ;
...
sig.autoSta_s(ToDelete)=[];sig.autoEnd_s(ToDelete)=[];sig.mark(ToDelete)=[];
